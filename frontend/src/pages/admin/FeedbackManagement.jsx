...
export default function FeedbackManagement() {
  ...
  const fetchFeedbacks = async () => {
    ...
    // ✅ Logic updated to use 'status' text column
    if (filter === "resolved") {
  ...
  const markResolved = async (id) => {
    ...
    // ✅ Database Update: setting status = 'resolved'
    ...
    if (error) {
      alert("Error updating DB. Check permissions.");
      fetchFeedbacks(); // Revert on error
    }
  };

  const markUnresolved = async (id) => {
    setFeedbacks(prev => prev.map(f => f.id === id ? { ...f, status: 'pending' } : f));
                  ...
                  <tr key={item.id} className={isResolved ? "row-resolved" : ""}>
                    <td>
                      <div className="student-info">
                        ...
                        {/* ✅ Using roll_no instead of email */}
                    ...
                    <td>
                      <div className="action-buttons">
                        {!isResolved ? (
                          <button onClick={() => markResolved(item.id)} className="button-icon confirmed" title="Resolve">
                             <Icon path={ICONS.resolve} />
                          </button>
                        ) : (
                          <button onClick={() => markUnresolved(item.id)} className="button-icon warning" title="Unresolve">
