...
const API_BASE_URL = "http://localhost:5000/api/rooms/allocations";
...
export default function VacantRooms() {
  ...
  const fetchAllocations = async () => {
    setLoading(true);
    try {
      // ✅ Now calling your Express API instead of Supabase directly
      const response = await fetch(`${API_BASE_URL}?hostel=${hostel}&floor=${floor}`);
  ...
  const isBedBooked = (currentRoomNo, bedIndex) => {
    ...
    return allocations.some((alloc) => {
        return String(alloc.room_number) === currentRoomNo && 
